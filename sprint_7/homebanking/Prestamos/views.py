...
def loan(request, client_id):
    ...
    cuenta = Cuenta.objects.filter(client_id__exact=client_id, tipo_id__exact=1).first()
                ...
                if datetime.strptime(fechaRecibida, '%Y-%m-%d') > datetime.today():
                    if cliente.tipo_id == 1:
                        if int(montoRecibido) <= 100000:
                            create_prestamo_and_update_balance(tipoRecibido, fechaRecibida, montoRecibido, "Aprobado", cliente.id)
                            ...
                            create_prestamo_and_update_balance(tipoRecibido, fechaRecibida, montoRecibido, "Pendiente", cliente.id)
